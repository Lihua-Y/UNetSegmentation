...
def per_class_recall(hist):
    ...
    for i in range(num_classes):
        true_positives = hist[i, i]
        false_negatives = np.sum(hist[i, :]) - true_positives
        recalls[i] = true_positives / max(true_positives + false_negatives, 1e-6)
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    return np.array(hist, astype(int)), IoUs, PA_Recall, Precision
